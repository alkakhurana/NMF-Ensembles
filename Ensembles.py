##Code for NMF ensemble based on different combinig methods and
...
## This corresponds to variable l in the paper
## and results are shown for l (ensemble size) = 2,4,6,8,10,20,30,40,50,100 
ensemble_size = 2
...
def generateBaseModels(A,k):
    ...
    ## numpy array for NMF-TR sentences score for all base models
    scores_TR = np.zeros((ensemble_size, A.shape[1]), dtype = 'float64')

    ## numpy array for stack of W matrices
    W_Stack = np.zeros((ensemble_size*k, A.shape[0]), dtype = 'float64')
    
    index = 0

    ## Loop to generate base models
    ## Number of base models generated are equal to
    ...
    return(scores_TR,W_Stack) ## returns NMF-TR scores of sentences for base models and stacked W matrix.
...
for f in os.listdir(".\\Documents"):
    ...
    [scores_TR, W_Stack]= generateBaseModels(A,k)
